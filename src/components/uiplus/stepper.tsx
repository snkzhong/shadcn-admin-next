// src/components/uiplus/stepper.tsx
...
export type StepperCircleStepIndicatorProps = {
  currentStep: number;
  totalSteps: number;
  size?: number;
  strokeWidth?: number;
...
type AsChildProps<T extends React.ElementType> = React.ComponentPropsWithoutRef<T> &
  React.PropsWithChildren<{
    asChild?: boolean;
  }>;

/**
 * 泛型类型：用于 defineStepper 返回值
 */
...
function getStepState(currentIndex: number, stepIndex: number): "active" | "completed" | "inactive" {
...
function onStepKeyDown(
  ...
  const direction = directions.next.includes(key)
    ? "next"
    : directions.prev.includes(key)
    ? "prev"
    : null;
...
const CircleStepIndicator = ({
  ...
  size = 80,
  strokeWidth = 6,
}: StepperCircleStepIndicatorProps) => {
...
// ========================================================
// 🔹 导出 Stepper 对象（支持 JSX 写法：<Stepper.Step />）
// ========================================================

/**
 * 命名空间式对象（供 JSX 使用）
 * 注意：实际组件将在 defineStepper 中动态返回
 */
export const Stepper = {
  // 占位符 —— 真实的 Stepper 是由 defineStepper() 返回的
  // 我们不在此处定义具体实现，而是让用户通过 defineStepper 创建实例
} as const;
